...
$stmt = $con->prepare("SELECT * FROM mmRelationship_tbl WHERE rejectDate IS NOT NULL OR endDate IS NOT NULL");
$stmt->execute(array());
$string = <<<EOT
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	if ($mentorRow['active'] == 1 && $menteeRow['active'] == 1) {
		$string = $string."<tr><td>".$mentorFullName."</td><td>".$row["mentorID"]."</td><td>".$menteeFullName."</td><td>".$row["menteeID"]."</td><td>".$requester."</td><td>".$row["requestDate"]."</td><td><a href='pairing.php?mentor=".$row["mentorID"]."&mentee=".$row["menteeID"]."&change=7' class='btn btn-outline-success'>Re-Request Pairing</a></td></tr>";
	}
}
print($string);